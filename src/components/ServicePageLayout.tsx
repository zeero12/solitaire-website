...
const SERVICES = [
  // FIX 6: Established canonical service order for inter-service navigation
  ...
  { path: '/services/equity-slbm', title: 'Equity / SLBM' },
  { path: '/services/bonds-and-fd', title: 'Bonds & FD' },
    ...
    <div className="bg-white min-h-screen font-sans text-gray-800">
      ...
      <section className="relative h-[60vh] min-h-[500px] flex items-center">
        <div className="absolute inset-0 z-0">
          ...
          <div className={`absolute inset-0 ${heroOverlayClassName}`}></div>
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="max-w-4xl">
            ...
            <div className="mb-6">
              <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-serif text-white leading-tight">{heroTitle}</h1>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center text-sm font-medium">
            <div className="flex-1 flex justify-start">
              {prevService && (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-white border border-gray-200 group-hover:border-brand-gold flex items-center justify-center transition-all duration-300 group-hover:scale-110 shadow-sm flex-shrink-0">
                    <ChevronLeft className="w-5 h-5 text-gray-500 group-hover:text-brand-gold transition-colors" />
                  </div>
                  <span className="text-sm font-medium tracking-wider uppercase hidden sm:block">{prevService.title}</span>
                </Link>
              )}
            </div>
            ...
            <div className="flex-1 flex justify-end">
              {nextService && (
                <Link 
                  ...
                  className="group flex items-center gap-2 text-gray-500 hover:text-brand-gold transition-colors"
                >
                  <span className="text-sm font-medium tracking-wider uppercase hidden sm:block">{nextService.title}</span>
                  <div className="w-10 h-10 rounded-full bg-white border border-gray-200 group-hover:border-brand-gold flex items-center justify-center transition-all duration-300 group-hover:scale-110 shadow-sm flex-shrink-0">
      ...
      {/* Intro Section */}
      <section className="pt-14 pb-20 lg:pb-32 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
            <div>
              <p className="text-gray-400 text-xs tracking-widest uppercase mb-4 font-medium">{introEyebrow}</p>
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-gray-900 leading-tight">{introHeading}</h2>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            <div className="lg:col-span-4">
              <div className="sticky top-24">
                ...
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-gray-900 leading-tight">{capabilitiesHeading}</h2>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            <div className="lg:col-span-4">
              <div className="sticky top-24">
                ...
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-gray-900 leading-tight">{promiseHeading}</h2>
              </div>
            </div>
            <div className="lg:col-span-8">
              {promiseHighlight && (
      ...
      <section className="py-20 lg:py-32 bg-gray-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-white leading-tight">{audienceHeading}</h2>
          </div>
      ...
      <section className="py-20 lg:py-32 bg-brand-light text-center relative">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-gray-900 mb-6">{ctaTitle}</h2>
          {ctaSubtitle && <p className="text-lg sm:text-xl text-gray-600 mb-10">{ctaSubtitle}</p>}
          <button 
            onClick={openModal} 
            className="bg-brand-blue hover:bg-[#152a45] text-white px-8 py-4 w-full sm:w-auto min-h-[44px] text-sm tracking-widest uppercase font-medium transition-colors inline-flex items-center justify-center gap-2"
          >
            {ctaText}
          </button>
        </div>
